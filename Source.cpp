...
struct Location
{
	float x = 0;
	float y = 0;
	std::vector<struct Card*>* lastLocation;
};
...
struct GameState
{
	Card deck[SUITESIZE][CARDSIZE];
	Card* drawnCards[3] = {};
	std::vector<Card*> slot[4] = {};
	std::vector<Card*> cardPiles[7] = {};
	std::vector<Card*> drawingDeck = {};
};
...
void Initialize(sf::RectangleShape cardStacks[7], sf::RectangleShape cardSlot[4])
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		cardStacks[i].setPosition(30.0f + CARDHEIGHT * (i + 1), 30.0f);
	}
	...
	for (int i = 0; i < SUITESIZE; i++)
	{
		...
		cardSlot[i].setPosition(SCREENWIDTH - CARDHEIGHT, 30.0f + ((CARDHEIGHT + 30.0f) * i));
	...
	for (int i = 0, k = 1; i < 7; i++)
	{
		for (int j = 0; j < k; j++)
		{
			...
			newLocation.x = (30.0f + CARDHEIGHT * (i + 1));;
			...
			cardPiles[i][j]->x = (30.0f + CARDHEIGHT * (i + 1));
			...
			if (length == 0)
			{
				...
				{	
					...
					for (int j = 0; j < heldCardStack.size(); j++)
					{
						...
						heldCardStack[j]->x = (30.0f + CARDHEIGHT * (i + 1));
...
void MouseClicked(sf::RenderWindow& window, sf::Vector2f translatedPos, sf::RectangleShape cardStacks[7])
{
	...
	if (drawingDeck.size() > 0 && drawingDeck.back()->cardBack.getGlobalBounds().contains(translatedPos))
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			drawnCards[i]->x = 30;
			drawnCards[i]->y = (CARDHEIGHT * 1.2) + (30 * i);
			drawnCards[i]->bIsFaceDown = false;

			Location newLocation;
			newLocation.lastLocation = NULL;
			newLocation.x = 30;
			newLocation.y = (CARDHEIGHT * 1.2) + (30 * i);
			drawnCards[i]->currentLocation = newLocation;
...
void DrawUIText(sf::RenderWindow& window, const std::string textString, sf::Vector2f textPos, int textSize, int drawDirection = 0)
{
	sf::Font font;
